...
- [`kboat-recall`](.claude/skills/kboat-recall/SKILL.md) — search your "read later" shelf by a question, over each source's saved `summary`/`topics`; also runs the routine's **daily pick**, surfacing up to two web reads matched to your `K-Boat Questions` Reminders backlog and recent Daily notes.
