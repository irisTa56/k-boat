...
The mechanical cores live in one tested Python package, [`kboat`](kboat/), rather than in prose, so the routine is cheaper and the logic is unit-tested. It exposes four tools over a shared frontmatter core and a code-authoritative schema (`kboat.schema`): `kboat-lifecycle` (the distill pass's cooldown clock and work-set predicates), `kboat-repos` (the repo catalogue's `gh` metadata gather, note writing, and full-catalogue refresh — which adopts repo renames automatically), `kboat-pick` (the daily pick's question/candidate gather and `picked` flag), and `kboat-validate` (checks every vault note against the schema). Its quality gate (ruff, `ty`, pytest) runs in pre-commit; invoke it with `mise run qa:py`, and autofix with `mise run fmt:py`.
