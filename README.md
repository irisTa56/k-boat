...
- `Reviews/` — one report per date on which a run distilled something, never two for a date, read for memory consolidation (the distillation knowledge only; operational detail stays in the run summary). Each carries a `read` flag you tick once you have read it.
...
- [`kboat-vault-conventions`](skills/kboat-vault-conventions/SKILL.md) — the shared vault mechanics every writer follows: URL-hash naming, the `kboat.schema` / `kboat-validate` contract, the `kboat.write.upsert` write contract, durability and the vault lock, and Base-authoring discipline. Both K-Boat and the feed-filter member defer to it.
...
- [`kboat-ingest`](skills/kboat-ingest/SKILL.md) — queue ingestion: draining the vault's `Queue/` folder (one `Queue/*.md` capture per URL, filled by the capture bookmarklet that `kboat-bookmarklet` prints) into source notes, each with its own 1:1 notebook; a GitHub repository's own URL is routed to `kboat-repos` instead, while a link deeper into a repository (an issue, a release, a file) stays a source.
- [`kboat-kindle`](skills/kboat-kindle/SKILL.md) — add a Kindle book from its `read.amazon` URL: it reads the metadata off the Amazon page through your own Chrome and writes the `Kindles/<ASIN>.md` note.
- [`kboat-repos`](skills/kboat-repos/SKILL.md) — catalogue a GitHub repository (and refresh the catalogue): it fetches metadata via `gh`, a cheap subagent judges role/domain/summary, and it writes the `Repos/<slug>.md` note.
- [`kboat-distill`](skills/kboat-distill/SKILL.md) — the post-reading pass: advancing lifecycle state, distilling ripe sources, and distilling ripe Kindle books from their highlights, into the knowledge graph. It defers to kboat-notes for the concept note's reading-group format and to the Basic Memory skills for the generic concept-note conventions.
- [`kboat-recall`](skills/kboat-recall/SKILL.md) — search your "read later" shelf by a question, over each source's saved `summary`/`topics`; also runs the routine's **daily pick**, surfacing up to two web reads matched to your `Questions.md` open-questions backlog and recent Daily notes, or time-sensitive to act on early (a security advisory, a release, a best-practice) regardless of theme.
- [`kboat-notebook-health`](skills/kboat-notebook-health/SKILL.md) — check that the sources you have opened still have their content. NotebookLM occasionally drops a source weeks after a clean ingest, leaving a notebook that looks fine and answers nothing; this adds the source back into that same notebook, so the dialogue you saved into it and everything else it carries stay where they are.
- [`kboat-rescue`](skills/kboat-rescue/SKILL.md) — clear a source out of the DLQ (a bot-protected PDF or a walled web page): finish it by pulling it through your real browser, or give it up where the page has died or you would rather not chase it.
- [`kboat-curate`](skills/kboat-curate/SKILL.md) — tidy the knowledge base on demand: curate the concept graph (orphans, duplicates, naming, relations) and check the concept-note tags for drift and gaps, against the KB's tag vocabulary.
- [`kboat-record-dialogue`](skills/kboat-record-dialogue/SKILL.md) — record in the knowledge base an `ask-kboat` answer you decided to keep: it checks and places each claim as `#dialogue` under a provenance line saying no source was read, and writes only what you confirm.
...
It exposes ten tools over a shared frontmatter core, a code-authoritative schema (`kboat.schema`), and a schema-driven writer (`kboat.write`):

- `kboat-lifecycle` — the distill pass's cooldown clock and work-set predicates.
...
- `kboat-concept` — classifies a concept note's `## Observations` into the shape the distill pass branches on before adding a reading group.
- `kboat-knowledge` — audits the knowledge base's concept notes for the curate pass: titles a wikilink cannot reach, and the facet-tag census.

The package's quality gate (ruff, `ty`, pytest, plus a per-file coverage floor) runs in pre-commit; invoke it with `mise run qa:py`, and autofix with `mise run fmt:py`.
