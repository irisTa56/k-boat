...
- `source wait` reported `.status: not_found` or `timeout`, or the `source get` type check itself failed. Transient, and pointedly **not** the DLQ — none of these says the fetch failed, so discard the unverified notebook (by the id `create` returned, or it leaks) and keep the reminder for the next run to redo the add.
