...
3. Create the source's 1:1 notebook (see kboat-notes "create or update a source note"): `create` → `source add` → **`source wait` to confirm NotebookLM fetched it** → write `notebooklm_id` and the derived `gemini_url`/`notebooklm_url` back onto the source note. If `source wait` does not reach `ready` (e.g. the site bot-blocks NotebookLM), keep the note and notebook but record the source as "added but not fetched" in the run summary — its notebook has no content to read or distill.
...
- Sources NotebookLM could not fetch or index (`source wait` not `ready`, e.g. bot protection). The note and notebook are kept, but the content is missing — report them so it can be supplied another way (e.g. uploading the PDF or text into the notebook by hand).
...
- Counts: items drained, notebooks created, sources left without a notebook, sources added but not fetched (blocked).
