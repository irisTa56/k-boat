...
- Counts: items drained, **GitHub repos catalogued** (routed to `Repos/`), notebooks created, sources left without a notebook, **sources sent to the DLQ** (blocked PDF, walled web page, or a web page NotebookLM typed `pdf` — all awaiting `kboat-rescue`), PDFs NotebookLM could not use (readable file, unusable notebook — the upload errored, or it extracted to empty/garbled text), and sources left with empty `summary`/`topics`. Also note any source NotebookLM typed outside the schema's two values (`youtube`, `epub`, …): it ingested fine and is kept as a `web_page`, so this is not an error — only a heads-up that its `source_type` is approximate. For PDFs also count: transient download failures (reminder kept) and titles that fell back to the reminder text.
