...
3. Create the source's 1:1 notebook (see kboat-notes "create or update a source note"): `create` → `source add` → **verify the fetch** (see kboat-notes: `ready` and the real article, not empty or a wall) → **verify the type** (see below) → capture `summary`/`topics` from the source guide and write them plus `notebooklm_id` and the derived `gemini_url`/`notebooklm_url` back onto the source note. Two verifications, two DLQ outcomes; run both in a cheap subagent, like the page fetch in step 1.
   - If the fetch is not successful (a wall), the notebook has no usable content: record the source in the DLQ (kboat-notes "Procedure: record a blocked source") — set `blocked: true` and discard the walled notebook — so `kboat-rescue` can supply the content later.
   - If `.source.type` from `source get` is `pdf`, step 1's sniff was fooled by a wall and this is not a web page at all. Record it in the DLQ as a `pdf` (`blocked: true`, `source_type: pdf`) and discard the notebook, so `kboat-rescue` supplies the file. The notebook reads fine here (NotebookLM's fetcher got through where ours did not), but the PDF path needs `PDFs/<slug>.pdf` and only a real browser can get it; a seconds-old notebook with no dialogue costs nothing to discard, and rescue rebuilds it from the file. Any other type is left alone and reported (see kboat-notes for why the check stays narrow to `pdf`).
...
- Web page NotebookLM typed `pdf` → the DLQ, as a `pdf` (`source_type` corrected, notebook discarded). Not a transient error and not a bug in the sniff: the URL carried no PDF marker and answered our GET with a wall, so nothing before the add could have typed it. `kboat-rescue` supplies the file.
- Source NotebookLM typed something other than `web_page` or `pdf` (`youtube`, `epub`, `docx`, …). Outside the schema's vocabulary and outside rescue's reach, so the source is kept on the web path as-is — report it so a human can decide.
...
- Errors: each collected error with the item it affected and the cause (e.g. bot-blocked PDF → DLQ, walled web page → DLQ, web page typed `pdf` → DLQ, an unsupported NotebookLM type, undecidable type, transient PDF download failure, rate-limited `create`/`source add`, persona-configure failure (non-fatal), source-guide failure, note write failure, slug collision).
