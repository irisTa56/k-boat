...
1. Decide the path, then gather signal. HEAD the URL to tell a PDF from a web page (`Content-Type: application/pdf`, or a `content-disposition` filename ending `.pdf`; fall back to the first bytes being `%PDF-`). The extension alone is not enough — an arXiv `/pdf/<id>` link has no `.pdf` suffix yet serves a PDF.
   - **Web page**: fetch the page with a cheap subagent to extract its title, summary, and topics. This gives more signal than the reminder title alone, and the fetches run in parallel. If the fetch fails (bot protection, HTTP 4xx/5xx, timeout), record the error and fall back to the reminder title.
   - **PDF**: follow kboat-notes "Procedure: ingest a PDF source" instead of steps 2–3 below. The title comes from the abstract page (arXiv) or the PDF itself, not an HTML fetch; downloading the file and creating the notebook are part of that procedure. Step 4 (deleting the reminder) still applies.
...
- Undecidable type: HEAD failed and the first bytes are not `%PDF-`, so neither path is safe to take. Skip the item and keep its reminder.
- PDF download failures (HTTP 4xx/5xx, timeout, zero size, or saved bytes not starting with `%PDF-`, e.g. an HTML error page). No note is written; keep the reminder.
...
- Counts: items drained, notebooks created, sources left without a notebook, sources added but not fetched (a web page blocked or walled, or a PDF that uploaded but did not extract). For PDFs also count: download failures, and titles that fell back to the reminder text.
- Errors: each collected error with the item it affected and the cause (e.g. bot-blocked fetch, undecidable type, PDF download failure, rate-limited `create`/`source add`, empty PDF extraction, note write failure, slug collision).
