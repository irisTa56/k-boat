...
3. Create the source's 1:1 notebook (see kboat-notes "create or update a source note"): `notebooklm --quiet create "<title>" --json` → `notebooklm --quiet source add "<url>" --notebook <id> --json` → write `notebooklm_id` and the derived `gemini_url`/`notebooklm_url` back onto the source note.
...
- Counts: items drained, notebooks created, sources left without a notebook.
