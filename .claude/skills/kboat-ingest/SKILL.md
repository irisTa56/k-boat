...
3. Create the source's 1:1 notebook (see kboat-notes "create or update a source note"): `create` → `source add` → **`source wait` to confirm NotebookLM fetched it** → write `notebooklm_id` and the derived `gemini_url`/`notebooklm_url` back onto the source note. If `source wait` does not reach `ready` (e.g. the site bot-blocks NotebookLM), or it reaches `ready` but the fetched text is a wall (login / JS-required / Cloudflare / paywall page) rather than the article, keep the note and notebook but record the source as "added but not fetched" in the run summary — its notebook has no usable content. Judge wall-vs-article by reading, not by keyword (page chrome like a `Log in` link is not a failure); distillation makes the same check, so catching it here just lets the content be supplied sooner. Run this fetch verification (`source wait` plus the wall check, reading the content via `source fulltext … -o`) in a cheap subagent — like the page fetch in step 1 — to keep the routine cheap.
...
- Sources NotebookLM did not properly fetch — `source wait` not `ready` (e.g. bot protection), or `ready` but the content is a wall (login / JS / Cloudflare / paywall) rather than the article (judge by reading; page chrome alone is not a failure). The note and notebook are kept, but the real content is missing — report them so it can be supplied another way (e.g. uploading the PDF or text into the notebook by hand).
