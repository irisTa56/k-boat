...
3. **Act on each judged entry** (one `feed-filter` process per entry — the write/record pair is atomic inside it).
   ...
   An apostrophe is ordinary in a title, and one left unescaped ends the quoting mid-value: the call never runs at all, so a Drop is never recorded and is re-judged every run after.
