...
Lead with what's actionable and **name the offending sites so they can be fixed or paused** — for a persistently erroring site, first check for a moved feed/index URL, then the remedy is `feed-filter disable-site --site-id <id>` (see the `kboat-manage-feed-sites` skill).
