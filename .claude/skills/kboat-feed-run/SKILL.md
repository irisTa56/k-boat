...
Send a push notification (the `PushNotification` tool — one line, ≤200 chars, no markdown) **only when the run produced something that needs your attention beyond the Feeds inbox**: a gather `error`, a self-heal, or an operational failure (a `remind` / `heal-site` non-zero exit, a missing-Playwright gate). Routine keeps and walls already land in the `Feeds/` notes where you'll see them in the Feeds Base, so a run whose only outcome is keeps/walls sends **no** push; a no-op run sends none either. This is the only channel for operational notices (none become feed notes, which are pages only). A `persistent == true` site is **always** such a case: send the push whenever any site is `persistent`, without exception — this is the escalation the durable counter exists to trigger, so it is not a judgment call, and lead the push with the persistent site and the URL-change recommendation from step 5. Lead with what's actionable and **name the offending sites so they can be fixed or paused** — for a persistently erroring site, first check for a moved feed/index URL, then the remedy is `feed-filter disable-site --site-id <id>` (see the `kboat-feed-manage-sites` skill). Example: `feed-filter: 2 site errors (<site> 403, <site> timeout), healed <site>`. Persistent example: `feed-filter: <site> gather failing 3 runs — check for a moved feed URL, else disable-site`. Always keep the fuller breakdown — including the keep/wall/drop counts — as the run's text output (the transcript), regardless of whether a push was sent; it is the record when the push is skipped or not delivered (the tool may report it wasn't sent; that is expected, no action needed).
