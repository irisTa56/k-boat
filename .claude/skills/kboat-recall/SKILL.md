...
2. If there are no `candidates`, run `kboat-pick set --slugs ""` to clear any stale `picked`, then stop and report zero picks. If there are candidates but neither `daily_notes` nor open questions, there is no interest signal — still run Stage 1 for act-early candidates only (step 3 infers no interests; a Tier 0 pick can fire on its own), and if Stage 1 finds none, clear `picked` and report zero picks.
...
4. **Stage 1 — local pre-filter (no NotebookLM).** Rank the candidates from their `summary`/`topics` alone (delegate to a cheap subagent when the inbox, the notes, or the question list are large, lexically pre-filtering first to keep it cheap — a Japanese note leans on the Japanese `summary`, since `topics` is English), in three tiers (an act-early tier above the two relevance tiers), and keep a **shortlist** of the top handful (about three to five — wider than the cap so Stage 2 has room to choose two), see kboat-notes "Daily pick":
   - **Tier 0 — act-early (time-sensitive, theme-independent)**: candidates whose value is perishable or front-loaded — acting sooner brings the benefit sooner — regardless of current interests: security advisories/vulnerabilities, releases and new features of tools or products, best-practice/tooling write-ups worth adopting now. Recognised from `summary`/`topics` alone; needs no interest match, leads the shortlist newest-`added_date` first, and may take a slot ahead of a better interest match (the booster that beats relevance).
   ...
   - **Tier 2 — same-field learning**: if Tiers 0–1 yield fewer than the shortlist size, extend with candidates in the broader field or theme of the open questions or the recent notes (not a direct match, but genuinely on-topic) — question fields first, then newest-note-first.
...
5. **Stage 2 — body-read final judgment (NotebookLM).** For each shortlisted candidate (a cheap subagent per candidate, in parallel), resolve its source id by matching its `url` (then `title`) in `notebooklm --quiet source list --notebook <notebooklm_id> --json` (`--quiet --json` so status output does not corrupt the JSON) — the original source, ignoring any saved dialogue notes (kboat-notes "One notebook per source") — then `notebooklm --quiet source fulltext <source_id> --notebook <notebooklm_id> -o <tmpfile>` (use `-o`, not stdout, which truncates at 2000 chars). Read the body, re-judge genuine relevance from it (demote or drop a candidate the summary oversold; confirm one that truly delivers), and record its length (the character count). Then choose at most two from the body-refined ranking, applying the two diversification preferences — see kboat-notes "Daily pick":
   ...
   - **Not two long reads**: estimate reading time from the body length, language-aware (~600 Japanese characters or ~250 English words — roughly 1500 characters — per minute; "long" ≈ 15 minutes or more). If the first pick is long, prefer a short second; never make both long while a short qualified candidate exists. An un-fetchable body has unknown length — neither preferred nor avoided.
   Both are tie-breakers only — never pull in a read that is neither on-interest nor act-early, never displace a clearly more relevant or more time-critical pick. Act-early picks fill slots first (up to both), then the interest tiers fill what remains. Stop at two; a candidate is picked at most once. If neither an act-early candidate nor anything in the field remains, take fewer — never pad with an unrelated read.
...
7. Report the picks — each with what it matched (the open question, the dated note, or the act-early reason: a security advisory, a release, a best-practice worth adopting now), so the inference is visible and checkable — and that they are read in the Today view of the reading-inbox Base (kboat-notes "Reading inbox Base").
