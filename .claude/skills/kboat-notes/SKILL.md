...
The tables' field list and order are checked against `kboat.schema` by `tests/test_doc_schema_sync.py` (run in `qa:py`), so adding, removing, renaming, or reordering a field on one side without the other fails the commit — the per-field prose (defaults, kinds, enums) is not machine-checked, so keep those accurate by hand.
...
3. Create the 1:1 notebook and record its coordinates:
   ...
   - Confirm it is a web page at all: once `ready`, read `.source.type` from `notebooklm --quiet source get <source_id> --notebook <id> --json`. This is NotebookLM's own verdict on the bytes its fetcher retrieved, in the same vocabulary as this schema (`web_page`, `pdf`), so it settles what the ingest sniff could only provisionally decide (see "Procedure: ingest a PDF source"). A `pdf` here is a **blocked PDF**, not a relabelling: reaching this step at all means our own fetch never saw `%PDF-` bytes (they would have taken the PDF path instead), so the file cannot be downloaded, which is exactly what the DLQ is for. Record it in the DLQ as a `pdf` source (see "Procedure: record a blocked source"), and `kboat-rescue` supplies the file. Do **not** widen this to "any type that is not `web_page`": the schema has only these two values and rescue can supply only a PDF or captured text, so any other type (`youtube`, `epub`, `docx`, …) stays on the web path and is reported instead.
...
**The sniff is not the last word.** Its two inputs — the bytes and the URL shape — can both come up empty at once: a host that serves a PDF from a URL carrying no PDF marker (e.g. preprints.org `/manuscript/<id>/download_pub`) and answers the browser-UA GET with a wall shows neither `%PDF-` nor a PDF endpoint, so the last rule types it a web page and it is ingested as one. Nothing about the URL can catch this — the only client that saw the real bytes is NotebookLM, whose fetcher is not blocked where `curl` is. So the web path re-checks the type against NotebookLM's own after the source is added (step 3 of "create or update a source note"), and that check, not this sniff, is what finally settles `web_page`-vs-`pdf`. The rules above stay the fast path: they decide the common cases before a notebook is created, and only a URL that defeats both of them pays for the round trip.
...
The DLQ is for sources whose content the unattended fetch could not get — the article text for a web page, the file bytes for a PDF — where pulling it through a real browser (`kboat-rescue`) is the fix. Three cases reach it:
...
- A **PDF the sniff could not see**: a walled URL carrying no PDF marker took the web path, and NotebookLM's post-add type check came back `pdf` (see step 3 of "create or update a source note"). Recorded as `pdf`, since a source typed here is by construction one whose bytes our fetch never got.

The third case reads fine in its notebook (NotebookLM's fetcher is not blocked where `curl` is) and is a DLQ entry anyway, because it is a PDF and the PDF path needs the **file**: `PDFs/<slug>.pdf` is the durable reading copy, so relabelling alone would leave a `pdf` source without one, and only a real browser can get those bytes. Discarding the working notebook costs nothing: at ingest it is seconds old and holds no dialogue, and rescue rebuilds it from the file.

(An uploaded PDF that extracts to nothing is *not* a DLQ case: its file is fine and readable, the notebook is just unusable — re-fetching the same file would re-fail, so it is reported, not blocked; see the PDF procedure.) Ingest does not drop a fetch-blocked source; it parks it in the DLQ:

1. Ensure `Sources/<slug>.md` exists with `blocked: true`, via `kboat-note write --type source` (slug = the url-hash, as in step 1 of the create procedure): a `{slug, fields}` record with `type: source`, `title`, `source_type`, `url` = the queued URL, `reading_link` = `url` (so a click goes to the original where the human can clear the wall themselves), and `blocked: true`. The tool creates the note if absent, or merges onto a note the web path already wrote before verifying; either way the DLQ entry exists with its `url` preserved. For the third case above the merge also carries `source_type: pdf`, correcting the sniff: it is the one path where a note's `source_type` changes after it is written.
2. Discard any notebook that was created per "discard a source's notebook", leaving `notebooklm_id` empty — rescue creates a fresh one. This covers a walled web fetch (contentless) and a web-path source NotebookLM typed `pdf` (readable, but rebuilt from the file). A fetch-blocked source has no local file (the fetch never produced one).
