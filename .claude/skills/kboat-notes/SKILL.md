...
The daily-pick step turns "what do I read first?" from scanning a flat inbox into a pull: the routine reads what you have been writing in your Daily notes, infers what you are currently interested in, and surfaces the sources that fit.
Two pieces implement it: the `kboat-pick` tool does the deterministic I/O — `kboat-pick candidates` reads the recent Daily-note bodies and the web inbox, `kboat-pick set` writes `picked` — and `kboat-recall` infers the interests and ranks the candidates between them. The routine runs the whole step after distillation; this section is the spec all three follow.

**Input — your recent Daily notes, no prescribed format.** The step reads the bodies of your Daily notes (in the `Daily/` folder, named `YYYY-MM-DD.md`) within the look-back window — whatever you wrote, in any structure. There is no required heading: an explicit question or wish becomes the strongest signal, but a passing note of a topic you are chewing on counts too, and `kboat-recall` infers your interests from that content. Pure logbook entries (done tasks, schedules, unrelated journaling) are meant to be ignored, not ranked on.
...
**Output — at most two `web_page` picks, marked `picked`.** `kboat-recall` infers your current interests from the in-window Daily notes, then ranks the active web inbox (`!distill && !keep && !dismiss && !blocked && source_type == "web_page"`, the candidate set `kboat-pick` returns) against them, reading the candidates' `summary`/`topics` and judging genuine relevance (pre-filtering lexically first when the inbox is large — against a Japanese note that pre-filter leans on the Japanese `summary`, since `topics` is English), in two tiers:

- **Tier 1 — direct interest.** Newest-note-first, take candidates that directly match an interest the notes express — an explicit question answered, or a clear current topic. Accumulate distinct picks until two.
- **Tier 2 — same-field learning, to fill the cap.** If Tier 1 yields fewer than two, top up with candidates that, while not a direct match, would teach you something in the field or theme of the recent notes — newest-note-first again. This is the deliberate relaxation: the pick should not stay empty when a genuinely on-topic read is sitting in the inbox.

Two is a cap, not a quota: stop at two, and if even Tier 2 finds nothing in the notes' field, take fewer rather than pad with an unrelated read — an honest short list beats a forced one. A candidate is picked at most once (Tier 1 wins over Tier 2 for the same source).
Each pick is reported with the inferred interest and the dated note it came from, so the inference — now implicit rather than a literal question — stays visible and checkable.
...
4. Once a real PDF is in hand the wall is cleared. Update the note with `kboat-note write --type source` — a `{slug, fields}` record carrying `blocked: false`, `notebooklm_id` and the derived `gemini_url`/`notebooklm_url`, `reading_link` = `[[<slug>.pdf]]`, and the captured `summary`/`topics` — merged over the DLQ note. The source leaves the DLQ and joins the inbox like a freshly-ingested PDF. Two non-clean endings: if the wall could not be cleared (no real PDF obtained), leave `blocked: true` — it stays in the DLQ; if the fetched PDF extracts to nothing, keep `blocked: false` (the fetch succeeded) but report the empty extraction — that is the ingest garbled-extraction case (readable file, unusable notebook), not a DLQ state.
