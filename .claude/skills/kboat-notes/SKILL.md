...
The diversification preference shapes *which* of the relevance-qualified candidates fill the two slots — it is a tie-breaker among on-interest reads, never an override: never pull in an off-interest candidate to satisfy it, and never displace a clearly more relevant pick for a worse one. It matters only when the tiers above qualify more than one candidate for a slot.

- **One older, one newer.** Split the candidate pool at the median `added_date`; once the first pick is chosen, prefer the second from the opposite age half, so the day's pair mixes a recent arrival with something that has been waiting. If only one half holds an on-interest candidate, take it rather than force the split.
