...
2. It prints a JSON report. The `kboat-repos` skill relays `adopted` (renames it healed), `rename_collisions` (a rename blocked by an existing note — a human merges), and `failed` (notes left exactly as they were — `gh` could not fetch them, their payload could not be read, or the rewrite failed) — the routine never deletes a note.
