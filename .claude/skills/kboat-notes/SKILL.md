...
2. **Re-create the notebook.** Re-run the matching ingest procedure's notebook step on the stored source — **step 3 of "create or update a source note"** for a web page, **step 5 of "ingest a PDF source"** for a PDF — and run it whole, down to the PDF `source rename` its title-keyed resolution later depends on. The note already holds everything that step reads (the `url`, the `title`, and for a PDF the file), and its write-back merges over the note, so nothing it does needs changing here. Restating a recipe instead would drift from it, and — restating the happy path, as a recipe does — would leave a failed reactivation with no ending at all. Those endings are what this step needs most: the ones that discard do so **passing the id `create` returned**, which is exactly right here, since the notebook this step just made is referenced by nothing — the note's `notebooklm_id` is empty either way, cleared by the earlier discard or never written at all — so a discard that read the note would find it empty and leak the notebook.
   ...
   Step 3 below belongs to the one ending that gives reactivation what it is for: a notebook whose original source has **usable text**, which is what dialogue and distillation — the two things a notebook is for, per the opener above — both need. Stop at every other ending, reporting where the source landed instead of choosing a disposition on it. Two are worth naming.
   ...
   - A PDF's **empty or garbled extraction** is the ending most easily mistaken for success: it reaches `ready`, keeps its notebook, and records a `notebooklm_id`. Judge it by usable text, not by any text — a garbled extraction has plenty, and it clears a "has text" gate while carrying neither dialogue nor distillation, exactly as an empty one does. Never set `distill` on either: distillation re-checks the extraction, and an unusable original aborts the source without stamping or discarding, so it would stay ripe and re-abort every run, forever. What it needs is a text-bearing copy — the empty-extraction remedy, not the `error` branch's different-or-re-exported one, which step 5 of "ingest a PDF source" separates and explains.
   ...
   - A **PDF arriving from an `error` branch** still holds at `PDFs/<slug>.pdf` the very bytes NotebookLM refused, so re-adding it unchanged only errors again. Replace that file with the different or re-exported copy first; until one exists there is nothing here to reactivate, which is why the `error` branches send the human for a copy rather than straight here.
