...
3. Build the notebook from the supplied content: `create` (read `.notebook.id`) → set chat persona (see "Procedure: set the notebook chat persona") → add the one source with `--title` set to the note's `title` (the added source has no `url` in either branch, so source-id resolution is title-keyed; for a PDF the durable title is finalised by the `source rename` below, since a file upload's `--title` does not stick), and read the returned source id from the `--json` output:
   ...
   - `not_found` or `timeout` → neither says the upload failed: `not_found` is a first-poll race against the source appearing (raised without retry), and `timeout` says only that we stopped waiting. On the ingest paths the fix is to let the next unattended run redo the add, but rescue has no next run and its upload came from a human-assisted capture, so **re-run the same `source wait` once** rather than throw that work away. If the retry is still not `ready`, take the unprocessable-upload ending in step 4.
   - `error` → NotebookLM took the upload and could not process it, durably: the library polls through an ERROR status only for a still-unclassified or media source and treats it as terminal for every other type — an uploaded PDF and a pasted-text source alike. Take the unprocessable-upload ending in step 4.
...
4. The wall is now cleared. Update the note with `kboat-note write --type source` — a `{slug, fields}` record carrying `blocked: false`, `notebooklm_id` and the derived `gemini_url`/`notebooklm_url`, the captured `summary`/`topics`, and `reading_link` (`[[<slug>.pdf]]` for a PDF; left as the `url` for a web page) — merged over the DLQ note. `source_type` is unchanged. The source leaves the DLQ and joins the inbox like a freshly-ingested source.
   ...
   - **Unprocessable upload** (step 3 read `.status: error`, or was still not `ready` on its retry): the notebook is discarded and there is no `notebooklm_id`, so what the source still has decides the note — judged by what its path requires, the file for a PDF and the article inside the notebook for a web page (the DLQ's own test, see "Procedure: record a blocked source (DLQ)"). The branches part ways here:
     - **PDF** → the real file is at `PDFs/<slug>.pdf`, which is exactly what the PDF path requires, so the wall *is* cleared and this is no DLQ state: write `blocked: false` and `reading_link` = `[[<slug>.pdf]]`, leave `notebooklm_id` and `summary`/`topics` empty, and report it. It lands where an ingest-time `error` lands — a readable PDF with no notebook — and re-uploading the same bytes would fail the same way, so rescue is not the fix; a text-bearing copy from a human is.
     - **Web page** → neither the article in the notebook nor a local copy (its reading copy was to be the still-walled `url`), so it keeps `blocked: true`. Rescue remains its fix: the captured text is re-derivable, so a different capture — or the skill's manual fallback, a hand-saved file — may upload cleanly.
...
1. Resolve the notebook id, which is not always on the note. A discard **after** ingest (a `dismiss`ed or distilled source) reads `notebooklm_id` from the source note. Every discard **while a notebook is being built** runs before the note has ever carried one, since `notebooklm_id` is written only after the checks pass, so the caller passes the id `create` returned. That is all three build paths — the DLQ and transient branches of "create or update a source note" (step 3), the `error` and transient branches of "ingest a PDF source" (step 5), and the non-`ready` endings of "rescue a blocked source" (step 3, where the DLQ note's `notebooklm_id` is empty by definition). With neither, the notebook is already gone — nothing to do. Never conclude "already gone" from an empty `notebooklm_id` alone when the caller has an id in hand: nothing else references that notebook, so skipping the delete leaks it where no vault check can ever see it.
