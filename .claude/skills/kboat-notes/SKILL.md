...
| `blocked` | Boolean, default `false`, managed by the routine — not the human. Set `true` when ingest could not get usable content (a bot-blocked PDF, a walled page, an empty/garbled extraction); the note then sits in the DLQ with `notebooklm_id` empty until `kboat-rescue` supplies the content and clears it. Always present (like `distill`) so the boolean Base filters never hit a missing property. |
...
When ingest cannot get usable content — a bot-blocked PDF (detection returned HTML for a `.pdf` URL), a web page NotebookLM fetched as a wall, or a PDF that uploaded but extracted to nothing — it does not drop the source; it parks it in the DLQ:

1. Ensure `Sources/<slug>.md` exists (slug = the url-hash, as in step 1 of the create procedure). If a note was already written before the failure (the web and PDF paths write it before verifying), update it; otherwise create it now with `source_type`, `url` = the queued URL, `read`/`done`/`distill` = `false`, `summary`/`topics` empty. Set `reading_link` = `url`, so a click goes to the original where the human can clear the wall themselves.
2. Set `blocked: true`. Discard any contentless notebook that was created (a walled web fetch) per "discard a source's notebook", leaving `notebooklm_id` empty — rescue creates a fresh one. A garbled-extraction PDF keeps its `PDFs/<slug>.pdf` file.
...
2. Obtain the content. For a PDF, get the real file to `PDFs/<slug>.pdf` — typically by driving the real browser (the `kboat-rescue` skill uses Claude in Chrome) to the `url`, letting the human solve any CAPTCHA, and saving the PDF; or by the human downloading it and pointing the skill at the file. Verify it starts with `%PDF-`. (A garbled-extraction case needs a cleaner file, not the same one re-fetched.)
...
4. On success, set `blocked: false`, write `notebooklm_id` and the derived `gemini_url`/`notebooklm_url`, and set `reading_link` = `[[<slug>.pdf]]`. The source leaves the DLQ and joins the inbox like a freshly-ingested PDF. If the fetch still fails, leave `blocked: true` and report it — it stays in the DLQ.
