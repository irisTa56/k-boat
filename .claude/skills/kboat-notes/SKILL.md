...
| `blocked_has_notebook` | `notebooklm_id` | `blocked: true` on a source that still carries a `notebooklm_id`. A DLQ entry has no notebook by definition (see "The DLQ"). |
...
| Field | Meaning |
| --- | --- |
| `blocked_count` | Sources in the DLQ (`blocked: true`). A DLQ nobody drains is what this makes visible: `kboat-rescue` is human-run, so nothing else would raise its hand. |
...
The DLQ is for sources whose ingest could not obtain what their path requires — the article inside the notebook for a web page, the file at `PDFs/<slug>.pdf` for a PDF — and could not for a durable reason, where supplying it through a real browser (`kboat-rescue`) is the fix. Four cases reach it, and no others. A merely transient failure is never one of them — it keeps its queue file and retries (see step 3 of "create or update a source note").
