...
| `url` | The URL the source was queued with, stored as given — a PDF's included, and kept through a rescue. The one rewrite is a GitHub file link, stored as the rendered page or download URL `kboat-repos gather` returns. The slug hashes its canonical form, but the value itself is not canonicalized. Immutable, and never empty on either type: it is the note's identity and its only route back to the page, so a note without one can no longer be shown to be about anything, a re-capture of the same page cannot de-duplicate against it, and a distillation of it writes observations with no provenance to carry. A PDF is the half that cannot be recovered from — its file is on disk and its page is not — which is why the type does not excuse an empty one. |
