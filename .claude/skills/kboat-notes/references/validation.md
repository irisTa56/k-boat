...
| `source_missing_url` | `url` | A source of either type with an empty `url`. Every procedure writes a source's queued `url`, a PDF's included, and the schema admits an empty one, so this rule is what reports the gap. It is the note's identity and its only route back to the page: the slug is that URL's hash, so a note without one can no longer be shown to be about anything, a re-capture of the same page cannot de-duplicate against it, and a distillation of it writes concept observations with no provenance to carry. Naming a type in the rule would leave the half that cannot be re-fetched — a PDF, whose file is on disk and whose page is not — reported by nothing. |
