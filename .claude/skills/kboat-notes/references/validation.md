...
| `blocked_has_notebook` | `notebooklm_id` | `blocked: true` on a source that still carries a `notebooklm_id`. Ingest leaves a DLQ entry without one (see [The DLQ](source-note.md#the-dlq-blocked-sources)), and its de-dup stops a re-capture of a source that has its notebook before anything can record it blocked ([create or update a source note](procedures.md#procedure-create-or-update-a-source-note), step 1). It can still arise by hand edit or on an entry recorded before that stop, so it stays checked, and [Procedure: abandon a blocked source](procedures.md#procedure-abandon-a-blocked-source) gates on it: clearing `blocked` silences this rule and can put the notebook in line for deletion. |
...
| `web_missing_url` | `url` | `source_type: web_page` with an empty `url`. Every procedure writes a source's queued `url`, a PDF's included, but the schema admits an empty one; this rule reports it on a web page, which is read from it. |
...
| `ripe_undistilled` | Sources the lifecycle calls **ripe** (`distill && !dismiss && !blocked`, cooldown elapsed, `distilled_date` empty). Distillation runs before validation, so after a run whose distillation completed this is what that run left ripe: sources whose concepts its create cap deferred or whose writes did not land, and sources it failed on. Its run summary names each with the reason, and says which of them need a human. |
| `ripe_undistilled_kindles` | Kindle books the lifecycle calls ripe (`distill`, `distilled_date` empty) — the same question for the other distillable kind, so a stalled Kindle pass is not invisible. No cooldown gates a book, so one still ripe after a run is one the run did not finish, for either of the reasons above. |
