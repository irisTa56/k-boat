...
So is one whose `owner/repo` looks ordinary and which GitHub then answers for with no repository, the reserved-route list being a cheap filter rather than the decision: `gather` asks `gh` and returns the same `skip-not-a-repo`.
