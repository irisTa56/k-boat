...
This is the web-page path.
For a PDF source, follow [Procedure: ingest a PDF source](#procedure-ingest-a-pdf-source) below, which shares step 1 (slug and de-dup) but differs in how the source note and notebook are built.
...
A PDF source is read inside Obsidian (the vault syncs to every device) and uploaded into its own notebook as a file, so neither Google Drive nor Google Play Books is involved — Play Books has no personal-upload API, and adding Drive buys nothing once Play Books is out.
The `url` is still the canonical de-dup key, so step 1 below runs the same de-dup as [create or update a source note](#procedure-create-or-update-a-source-note); only the later steps differ.

One source reaches this path with its type already decided: a GitHub blob/raw `.pdf` link, which `gather` routes here as `source-file` (`source_type: pdf`) with `url` already rewritten to its `raw.githubusercontent.com` download URL (see [Naming and de-dup](repo-note.md#naming-and-de-dup) under the repo schema).
...
The bytes decide PDF-vs-not; once the bytes are HTML, the URL shape and the response together decide blocked-vs-web.
A `.pdf` extension on the final segment is enough on its own — any HTML for it is a challenge served in place of the file.
A bare `/pdf/` segment is not: a docs page like `…/guide/pdf/overview` legitimately serves a `200` HTML article and must read as a web page, so `/pdf/` flags a blocked PDF only when the response is itself a bot challenge (the ACM `/doi/pdf/<doi>` case, where Cloudflare answers the browser-UA GET with a `403` `Just a moment…` interstitial).
This keeps a true but walled PDF endpoint off the web path even without a `.pdf` suffix, while a real PDF endpoint that is *not* walled, like arXiv `/pdf/<id>`, returns `%PDF-` bytes and is caught by the first rule.
Strong bot protection (e.g. scispace) blocks `curl` even with a browser UA and answers unreliably; such a source is reported as blocked, not worked around.
The browser UA still matters: many hosts gate the file on it and serve it fine once it is set.

**The sniff is not the last word.**
Its two inputs — the bytes and the URL shape — can come up empty at once: a host that serves a PDF from a URL carrying no PDF marker (e.g. preprints.org `/manuscript/<id>/download_pub`) and answers the browser-UA GET with a wall shows neither `%PDF-` nor a PDF endpoint, so the last rule above types it a web page.
Nothing about the URL can catch that; the only client that saw the real bytes is NotebookLM.
So the web path re-checks the type against NotebookLM's own once the source is added (step 3 of [create or update a source note](#procedure-create-or-update-a-source-note)), and that check is what finally settles `web_page`-vs-`pdf`.
...
Every source — web or PDF — gets a `summary` and `topics`, captured at ingest while its notebook still exists.
They are the durable, searchable description the recall skill leans on once the notebook is discarded, and they make the Base browsable.
...
Every 1:1 notebook gets the same chat persona, set once right after `create`, so reading-time dialogue stays honest and direct instead of agreeable.
NotebookLM and Gemini share one per-notebook custom instruction, so this single setting governs both the NotebookLM chat and the Gemini view (`gemini_url`); the output language is left to NotebookLM's own language setting, not the persona.
The persona is a per-notebook setting persisted on the notebook (it survives across chats), not a per-message flag.
Set it with `notebooklm configure --notebook <id> --persona "<persona>"` (the bare `--persona` selects NotebookLM's "Custom" goal).
...
**What governs is what survived, not which check said no.**
Step 2 imports the ingest verification whole, so every ending that step has arrives here, and the cases below are the ones that arise rather than the whole list: where the notebook survives and the article can still be got by hand the way on is the browser capture, where the notebook itself cannot serve it is reactivation, and where neither is true there is none to name.
...
What decides is whether a notebook can serve, never whether one exists.
A `keep` source keeps its notebook and a `keep` disposition alone changes nothing about it, so where that notebook still answers, reading, dialogue, and distillation need no reactivation there — adjust the disposition, and for a re-distillation clear `distilled_date` along with it, that stamp being terminal and no disposition overriding it.
Reactivation is for a source with **no usable notebook** — none at all, or one that can serve neither dialogue nor distillation, which a `keep` source is as able to hold as any other.
Sources arrive holding **no notebook** in two ways from outside this procedure.
One is a notebook discarded *after* it did its job: a `distill`-only source after distillation, or a `dismiss`ed one.
The other is narrower than "any failed ingest": only a **PDF left with its file but no notebook**, from the `error` branch of [ingest a PDF source](#procedure-ingest-a-pdf-source) (step 5) or the PDF half of rescue's notebook-not-built ending.
Sources arrive **with** one wherever a check condemned text the notebook already held and it was kept anyway, each on its own reason.
A **PDF whose extraction was empty or garbled** (step 5 of [ingest a PDF source](#procedure-ingest-a-pdf-source), or rescue's ending of the same name) is kept so a human can weigh that verdict against the text it was made on.
...
Step 2 discards that notebook before rebuilding, keying on the note carrying a `notebooklm_id` rather than on how the source came by one, so nothing else here turns on which it is.
No other ingest or rescue ending arrives here, each having a better way back of its own: a DLQ entry rests there for `kboat-rescue` whatever its `source_type`, and a transient failure keeps its queue file for the next run.
A notebook that survives but has **lost its original source** does not arrive here at all, and it is worth saying where it goes instead, because its note reads exactly like a healthy one: the notebook is still there under its own id and title, and only a `source list` shows it holding zero sources (both observed losses), or nothing carrying a content type.
Nobody deleted it — NotebookLM has been seen dropping a web source weeks after a fully verified ingest.
Reactivation is the wrong instrument for it, and destructively so: the notebook is not what failed, and step 2 would discard it along with whatever dialogue it still holds.
[Procedure: restore a source's original into its notebook](#procedure-restore-a-sources-original-into-its-notebook) is that case, and it adds the missing source back without touching the notebook.
What arrives here from that direction is narrower: a **PDF** whose own file defeated the restore, and a source whose `notebooklm_id` names no notebook — the same population as the out-of-band deletion below, reached from the restore rather than from a sweep.
A **web** source whose `url` no longer yields the article does *not* arrive here, however plainly its restore failed — that procedure stops and leaves the reader to choose, because the way on is `kboat-rescue` and reaching it costs the notebook.
That population — a notebook deleted out of band, its `notebooklm_id` left pointing at nothing — step 2 takes without ceremony: `notebooklm delete` is idempotent by contract — deleting an already-absent notebook succeeds — and a stored id is a full UUID, which the CLI passes straight through rather than resolving against a listing, so the discard clears the coordinates as it would for any live notebook.
`kboat-distill` reports one as an anomaly when a ripe source hits it, the only sweep that would notice; this procedure is what settles it.
...
The DLQ is for sources whose ingest could not obtain what their path requires — the article inside the notebook for a web page, the file at `PDFs/<slug>.pdf` for a PDF — and could not for a durable reason — durability being the whole test, whether or not a real browser can go on to supply it.
Four cases reach it, and no others.
A merely transient failure is never one of them — it keeps its queue file and retries (see step 3 of [create or update a source note](#procedure-create-or-update-a-source-note)).
...
Discarding the working notebook costs nothing here — at ingest it is seconds old and holds no dialogue, and rescue rebuilds it from the file.
Mostly the wall is durable (the motivating host blocks `curl` however it is asked), but a source that landed here through a passing failure is not stranded either: the note keeps its `url`, so rescue completes it like any other.

(An uploaded PDF NotebookLM cannot use is *not* a DLQ case, whether it errors outright or reaches `ready` and extracts to nothing: its file is fine and readable, the notebook is just unusable — re-uploading the same bytes would re-fail, so it is reported, not blocked; see the PDF procedure.
...
Abandoning is **the human's call, never the routine's** — confirm it with them before writing, since it ends the source rather than deferring it.
...
**Name every disposition rather than only the one you want.**
`upsert` merges, and a DLQ entry's dispositions are inert rather than absent — a human who wanted the entry gone may have ticked one and watched nothing happen.
Any left to the merge lands beside `dismiss` as the `ambiguous` violation, trading ageing in `blocked_count` for a permanent finding in the daily report.

**Where `distilled_date` stands, write the other record**: `blocked` and `dismiss` false, `distill` and the stamp left alone, and for a `pdf` source `reading_link` = `[[<slug>.pdf]]` besides.
There is nothing left to abandon — the knowledge is already in the graph — so the source rests in Holding as the distilled source it is.
`keep` is deliberately not named: with `dismiss` false it composes with `distill` legitimately, and clearing it would take the source off the read-later shelf `kboat-recall` searches by default.
The `reading_link` matters for the same reason — only a re-captured source can carry a `distilled_date` into the DLQ, and recording that entry overwrote the link with the `url`, which would then be what recall offers as the way to read it.

**The record lands in one write**, `kboat-note write` taking one record and one `upsert` being one atomic write (see `kboat-vault-conventions`, "The write contract"), so no part of the set is ever on disk without the rest.
That matters because neither half stands alone: `dismiss: true` is inert while `blocked` stands, and `blocked: false` lands wherever the existing ticks leave it — no cross-field rule fires on any of those landings, so a half-write produces no signal that it happened.
...
Every view filters it out but Holding, and both recall's default scope and the daily pick exclude it.
...
This deletes the source's 1:1 NotebookLM notebook and clears its coordinates.
