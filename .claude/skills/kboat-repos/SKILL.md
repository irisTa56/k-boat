...
2. **Relay** the report: counts (`total`/`updated`/`adopted`/`rename_collisions`/`failed`/`anomalies`), and the `adopted` (renames it healed: `was` → `now`, `from` → `to`), `rename_collisions` (a rename blocked because the canonical slug is already taken — a human merges the two notes), and `failed` (a note this run did not refresh) entries. The routine never deletes a note.
   ...
   Each `failed` entry carries a `reason` — one of five — and an `error` with the detail. Two of them need a human, and the other three are settled by the next run:
   ...
   - `note` — the note has no line for a field the refresh rewrites, so nothing was written. **Escalate it** too: a note's shape does not change on its own. One entry is a note to repair by hand; every note at once is a field added to the refresh without the catalogue being migrated to carry it.
...
- `refresh` `failed` entries (one note this run did not refresh — see "Procedure: refresh the catalogue" step 2 for the five `reason` values, and for the two that need a human rather than the next run: `payload` and `note`), `rename_collisions` (a rename blocked by an existing note), and `adopted` (renames healed) — surface them; never delete.
