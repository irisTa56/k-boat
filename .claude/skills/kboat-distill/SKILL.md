...
3. **Extract** (read-only, safe to repeat). On any extraction error, abort this source — do not stamp, do not discard — record the error and continue to the next source.
   - **Content**: write the full text to a temp file with `.venv/bin/notebooklm --quiet source fulltext <source_id> --notebook <notebooklm_id> -o <tmpfile>`, then read it. Use `-o` (not stdout, which truncates at 2000 chars); avoid `-f markdown` (it needs the uninstalled `markdownify` package and fails silently under `--quiet`). Confirm the file is the actual source, not just non-empty: if it is empty, or is really a login wall, JS/cookie-required stub, Cloudflare challenge, or paywall page that NotebookLM fetched instead of the article, abort this source as a fetch failure and report it. Judge by reading — page chrome such as a `Log in` link or a noscript `enable JavaScript` notice alongside the real text is normal, so keyword-matching alone gives false positives.
