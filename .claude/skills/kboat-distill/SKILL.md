...
- **Create only specific concepts.** Auto-create a standalone note only for a clearly named concept (an algorithm, system, protocol, paper).
  ...
  - Title it so the title is also its filename (kboat-notes [Concept notes](../kboat-notes/references/concept-notes.md#concept-notes-kboat_knowledge_path)): a title Basic Memory has to sanitize still resolves in Basic Memory but breaks every inbound link in Obsidian and Foam.
