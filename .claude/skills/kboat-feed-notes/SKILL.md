...
It is a "here is something worth a look" entry the human browses, reads on the spot, shelves, dismisses, or promotes to a full K-Boat read.
...
Every step has to be reachable without scrolling sideways, so a view carries only what that motion needs — the card, and the three checkboxes.
...
The views are `table`.
The lead column's width and the row height are tuned together — the column narrow enough that all three checkboxes fit beside it, the row tall enough that the narrowed card still shows in full — but both are per-vault tweaks, so the live `Feeds.base` carries a `columnSize` and `rowHeight: extra` not shown here.
