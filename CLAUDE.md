...
- A notebook is only useful if NotebookLM fetched the content, not a wall. Ingest verifies the fetch (`source wait` + a content check); a source it cannot **fetch** — a bot-blocked PDF or a walled page — is not dropped but parked in the **DLQ** (`blocked: true`, `url` kept, reminder deleted) for `kboat-rescue`. A PDF that fetched fine but extracted to nothing (an image-only scan) is *not* DLQ'd — the file is readable, only the notebook text is unusable, so it is ingested and reported, not blocked. (CLI specifics live in the skills.)
