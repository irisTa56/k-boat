...
- The NotebookLM source id is not stored.
  - It is a per-notebook attribute resolved on demand by matching `url` (then `title`) in `notebooklm source list`. A file-uploaded PDF source has `url: null`, so it matches by `title` — ingest passes `source add --title` set to the note's `title` so the match works.
