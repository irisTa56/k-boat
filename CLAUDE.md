...
- `feed-filter.db` (seen-store), `feed-filter-browser-state.json` (persisted browser cookies), and `sites.toml` (the personal subscription list — contains personal data) are **gitignored local state**; never commit them. Only `selection.md` is version-controlled config.
