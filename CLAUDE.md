...
- `kboat-pick` (`kboat.pick`) — the daily-pick mechanics: `candidates` (the recent Daily-note bodies over a `--lookback-days` window + the active web inbox, as JSON) and `set` (reset `picked`, then set it on the chosen slugs), no LLM. `kboat-recall`'s daily-pick mode ranks between the two. See the `kboat-pick` tool and kboat-notes "Daily pick".
...
- The NotebookLM source id is never stored — it is resolved on demand by matching `url` (then `title`, for `url: null` PDF uploads) in `notebooklm source list`; that match identifies the **original** source, and any other source in the notebook is saved reading-time dialogue, distilled as `#dialogue`.
...
- The daily pick is a routine step, not a disposition: each run resets the hidden `picked` boolean on every source and re-sets it on at most two `web_page` sources matched to interests inferred from two read-only signals — the `K-Boat Questions` open-questions backlog (the deliberate signal, unresolved Reminders items, priority-ordered) and the recent Daily notes over a bounded newest-first look-back (the ambient signal, last ~2 weeks, `kboat-pick candidates --lookback-days`) — ranked in two tiers (direct-interest first, then same-field learning to fill the cap, taking fewer rather than padding with an unrelated read), shown in the Reading Inbox Today view beside in-progress (`reading`) sources of any type. Both signals are read, never written; the mechanics are `kboat-pick` (and `rem` for the questions), the ranking `kboat-recall`.
