...
- The DLQ is exactly the *fetch*-blocked set: a source whose path needs content the unattended fetch cannot get — the article text for a web page, the file bytes for a PDF — becomes a `blocked` note keeping its `url` (not a silently re-failing reminder), cleared by `kboat-rescue`. A PDF that fetched but extracted to nothing is readable — ingested, not blocked. The type is not the URL's to decide: a walled PDF endpoint with no `.pdf` marker sniffs as a web page, so the web path re-checks NotebookLM's own `source get` type after the add and DLQs a `pdf` found there, correcting `source_type` — its notebook reads fine, but reaching that check means our fetch never saw the bytes the PDF path needs.
