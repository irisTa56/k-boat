...
- Quality gates (`mise run pre-commit` runs them all; the git pre-commit hook calls it, so a failure blocks commits):
  ...
  - `mise run qa:links:offline` — offline lychee pass over the repo (no network, `--hidden` so it reaches `.claude/skills`, plus `--include-fragments`); mirrors CI's `link-check-offline` job so a broken relative link is caught locally, not only in CI.
