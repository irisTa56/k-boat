...
Eight skills, all under `.claude/skills/`:
...
- `kboat-rescue` — interactive, Mac-only: completes a DLQ (`blocked`) source — a bot-blocked PDF or a walled web page — by pulling the content through the user's real Chrome (Claude in Chrome): a PDF is saved to `PDFs/` and uploaded, a web page is captured as a text upload (no local file, reading stays the `url`); keeping the `url` either way.
...
- Every source carries `summary` and `topics` (from the NotebookLM source guide at ingest, normalised to a fixed language — `summary` Japanese, `topics` English — aligning with a repo note and giving recall a bilingual lexical surface) — the durable description `kboat-recall` searches once the notebook is gone. A source-guide failure leaves them empty; `kboat-ingest` retries the capture each run (the `needs_summary` set) while the notebook still exists.
- The daily pick is a routine step, not a disposition: each run resets the hidden `picked` boolean on every source and re-sets it on at most two `web_page` sources matched to interests inferred from two read-only signals — the `K-Boat Questions` open-questions backlog (the deliberate signal, unresolved Reminders items, priority-ordered) and the recent Daily notes over a bounded newest-first look-back (the ambient signal, last ~2 weeks, `kboat-pick candidates --lookback-days`) — chosen in two stages: a local pre-filter ranks the active web inbox — undispositioned web pages you have not started, the Web view minus its in-progress (`reading`) reads — on `summary`/`topics` in three tiers into a small shortlist — an act-early tier (time-sensitive and theme-independent: security advisories, releases, best-practices worth adopting now, recognised from `summary`/`topics` and pickable regardless of interest, the one booster that beats relevance and the one case an off-interest read is surfaced) above two relevance tiers (direct-interest first, then same-field learning) — then a body-read stage fetches that shortlist's NotebookLM fulltext to re-judge relevance and pick at most two, diversified by two tie-breakers over the qualified set (one older + one newer by `added_date`, and avoiding two long reads via the body length). Only the shortlist is ever fetched, and a NotebookLM-down day degrades to the Stage 1 `summary`/`topics` ranking (still producing a pick). The picks show in the Reading Inbox Today view beside in-progress (`reading`) sources of any type. Both signals are read, never written; the mechanics are `kboat-pick` (and `rem` for the questions), the ranking and body read `kboat-recall`.
