...
- Every source carries `summary` and `topics` (from the NotebookLM source guide at ingest, normalised to a fixed language — `summary` Japanese, `topics` English — aligning with a repo note and giving recall a bilingual lexical surface) — the durable description `kboat-recall` searches once the notebook is gone.
