...
- The reading inbox is one vault-wide standalone Base (`Reading Inbox.base`) with to-read views split by `source_type` — web, PDF, and a catch-all so no unread source is ever hidden — and a Processed view that exposes the lifecycle state.
