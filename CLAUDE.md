...
- Quality gates (`mise run pre-commit` runs them all; the git pre-commit hook calls it, so a failure blocks commits):
  ...
  - `mise run qa:links:offline` — offline lychee pass over the repo (no network; scope from `lychee.toml`, which every invocation names with `--config`); mirrors CI's `link-check-offline` job so a broken relative link is caught locally, not only in CI.
