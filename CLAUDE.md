...
- `kboat-pick` (`kboat.pick`) — the daily-pick mechanics: `candidates` (the recent Daily-note bodies over a `--lookback-days` window + the active web inbox, each candidate carrying `summary`/`topics` and `added_date`, as JSON) and `set` (reset `picked`, then set it on the chosen slugs), no LLM. `kboat-recall`'s daily-pick mode ranks and diversifies between the two. See the `kboat-pick` tool and kboat-notes "Daily pick".
...
- The daily pick is a routine step, not a disposition: each run resets the hidden `picked` boolean on every source and re-sets it on at most two `web_page` sources matched to interests inferred from two read-only signals — the `K-Boat Questions` open-questions backlog (the deliberate signal, unresolved Reminders items, priority-ordered) and the recent Daily notes over a bounded newest-first look-back (the ambient signal, last ~2 weeks, `kboat-pick candidates --lookback-days`) — ranked in two relevance tiers (direct-interest first, then same-field learning to fill the cap, taking fewer rather than padding with an unrelated read), then diversified by a tie-breaker preference over the qualified set (one older + one newer by `added_date`), shown in the Reading Inbox Today view beside in-progress (`reading`) sources of any type. Both signals are read, never written; the mechanics are `kboat-pick` (and `rem` for the questions), the ranking `kboat-recall`.
