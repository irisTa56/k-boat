...
- `kboat.lock` — `vault_lock`, the vault-wide mutual exclusion every mutating run holds so two runs cannot interleave over one vault. A caller chooses between refusing a held vault at once and waiting a bounded few seconds; a lock a crashed run left behind is taken over as soon as the pid it names is gone.
