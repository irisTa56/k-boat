...
- Ruff is configured workspace-wide in the root `pyproject.toml`; `ty`, pytest and the coverage floor are per-member, in this package's own `pyproject.toml`. Run this member's gate with `mise run qa:py:kboat`.
