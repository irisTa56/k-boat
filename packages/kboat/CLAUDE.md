...
- `kboat.lock` and `kboat.io_utils` are the vault's concurrency and durability floor. Take the lock at a CLI edge, never inside a writer (it is not re-entrant), and never add a second file-writing path beside `atomic_write_text`.
