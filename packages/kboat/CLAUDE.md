...
- Ruff is configured workspace-wide in the root `pyproject.toml`; `ty` and pytest in this package's own. The binding coverage gate is the per-`src/`-file floor in `scripts/coverage_floor.py`, which `mise run qa:py:kboat` runs after pytest — pytest's `--cov-fail-under` is the package average, a looser and separate one.
