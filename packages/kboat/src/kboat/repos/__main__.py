...
- `backfill --dry-run|--apply` — write each backfilled field (`readme`, `gone`)
  a repo note has no line for, at its schema default.
...
deleted. `backfill` is the exception: it rewrites every note in the live
