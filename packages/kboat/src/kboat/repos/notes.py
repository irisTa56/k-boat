...
that `refresh` uses, which is this module. Lists (`language`, `topics`,
`domain`) are inline (`topics: [a, b]`), so every top-level field is one line
and `refresh` can rewrite a field by replacing that line.
"""
