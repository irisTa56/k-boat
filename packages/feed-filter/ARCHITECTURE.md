...
- `json_text.py` — the rule for a lone surrogate `json.loads` hands back, which every JSON reader (`discourse`, `exa`) applies to each string field at its read.
  - Display text is substituted, and identity text (a URL, a slug) is refused so the caller takes its fallback; the reasons are the module docstring's.
