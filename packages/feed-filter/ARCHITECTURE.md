...
- **A failed forum topic loses only itself.**
  ...
  - Every boundary re-raises `sqlite3.Error`, the forum counterpart of the article path's `MissingPlaywrightError` carve-out: a store error is scoped to a connection, table, or lock rather than to one site or topic, so absorbing it would report one store bug as N site outages — each with a rising failure counter that escalates healthy forums to `persistent`.
    ...
    - `main` reports only the one case it still expects — `seen.open_db` losing the migration lock's busy timeout, wrapped as `SeenStoreBusyError` — as `error: …` with exit 1; any other `sqlite3.Error` this re-raise carries up (a bad migration statement, a constraint violation) is a store bug of ours and now reaches a traceback instead of being read as the disposition commands' honest store failure (`cli.py`'s `main` docstring).
