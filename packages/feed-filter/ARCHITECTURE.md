...
- **A failed forum topic loses only itself.**
  ...
  - Every boundary re-raises `sqlite3.Error`, the forum counterpart of the article path's `MissingPlaywrightError` carve-out: a store error is scoped to a connection, table, or lock rather than to one site or topic, so absorbing it would report one store bug as N site outages — each with a rising failure counter that escalates healthy forums to `persistent`.
    ...
    - `main` reports a `sqlite3.Error` where `seen.is_environment_failure` is true — a lock timeout on any write to the seen-store (not only `seen.open_db`'s migration), or SQLite attributing the failure to permissions, memory, the disk, or file corruption — as `error: …` with exit 1 (`cli.py`'s `main` docstring). Any other `sqlite3.Error` this re-raise carries up (a bad migration statement, a constraint violation) is a store bug of ours and reaches a traceback instead.
