...
The heal is conditional on two things: that discovery read the page the gather reads — it fetches over plain HTTP, so for a `requires_browser` site it does not — and that an article cluster can be named in what came back. Where either fails the run reports the site with what discovery returned and leaves its config alone; the repair from there is not the run's.
