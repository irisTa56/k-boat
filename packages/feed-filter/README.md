...
Where discovery hands back no article cluster there is nothing to heal with, so the run reports the site with what discovery returned and leaves its config alone; the repair from there is registration work rather than the run's.
