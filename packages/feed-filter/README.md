...
The LLM owns the genuinely fuzzy judgments, among them picking what to register from discovery's candidates (at site registration, and again when a scrape site self-heals), authoring the descriptions the query gather searches on (ad hoc — no skill drives it yet), and per-page keep/drop selection.
