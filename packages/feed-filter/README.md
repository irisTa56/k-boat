...
The LLM owns only the genuinely fuzzy judgments, among them picking what to register from discovery's candidates (at site registration, and again when a scrape site self-heals), authoring the descriptions the query gather searches on (ad hoc — no skill drives it yet), and per-page keep/drop selection.
...
- `OBSIDIAN_VAULT_PATH` set (via the workspace `mise.local.toml`) to the Obsidian vault; kept entries are written as `Feeds/` notes under it. An unset vault path makes a write fail loudly rather than silently dropping kept entries.
...
It does not skip the pause either: nothing is gathered while a site is disabled, so whatever was published during the pause and is still in the feed or index is judged over the runs after it.
...
- Ensure `OBSIDIAN_VAULT_PATH` is set in the task's environment (loaded from the workspace `mise.local.toml`); an unset vault path surfaces as a non-zero exit, not a silent drop.
...
The heal is conditional on two things: that discovery read the page the gather reads — it fetches over plain HTTP, so for a `requires_browser` site it does not — and that an article cluster can be named in what came back.
Where either fails the run reports the site with what discovery returned and leaves its config alone; the repair from there is not the run's.
