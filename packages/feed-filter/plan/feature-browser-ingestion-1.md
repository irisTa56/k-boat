...
> **Post-implementation note (validated 2026-06-06).** All four phases shipped (PRs #2–#5). Live validation against the motivating site **Lab BRAINS** then reversed **REQ-004**: persisting `storage_state` across runs is *counterproductive* for the Cloudflare managed-challenge class — replaying `cf_clearance` / `__cf_bm` re-triggers an interactive "Just a moment…" challenge headless Chromium cannot solve (1/5 success), whereas a clean cold context with only the UA strip passes reliably (5/5). Persistence was therefore dropped (PR #6, cold context per process), which also voids **SEC-002** (no persisted-state file) and the `FEED_FILTER_BROWSER_STATE` half of **PAT-001**. **RISK-005** resolved favorably: Lab BRAINS' `/feed/` carries summaries rich enough to judge, so the per-article browser fetch (**ALT-002** / GOAL-005 follow-up) is *not* triggered. The reversed requirements below are marked inline; the rest stands as implemented.
...
- **REQ-004**: ~~Browser cookies are **persisted** to a gitignored local file (read on launch if present, written on close) so they carry across runs, with an env override for its path.~~ **[REVERSED 2026-06-06 — validation showed persistence is net-negative for the Cloudflare managed-challenge class; the context is now created cold per process. See the post-implementation note.]** No headed/manual seeding (see REQ-005).
...
- **SEC-002**: ~~The persisted browser-state file may contain auth/clearance cookies; it is gitignored local state and must never be committed (same class as `sites.toml` / `feed-filter.db`).~~ **[VOID 2026-06-06 — no persisted-state file exists after the REQ-004 reversal.]**
...
- **PAT-001**: ~~Browser state path resolves like `feed-filter.db`: a repo-relative default plus an env override (`FEED_FILTER_BROWSER_STATE`), centralized in `config.py`.~~ **[VOID 2026-06-06 — `browser_state_path` / `FEED_FILTER_BROWSER_STATE` were removed after the REQ-004 reversal.]**
...
|------|-------------|-----------|------|
| TASK-001 | `src/feed_filter/sites.py`: add `requires_browser: bool = False` to `SiteConfig`; parse it in `load_sites` via a strict bool helper (absent → False; non-bool → ValueError, ported from loose-feeds `_require_bool`); emit it in `add_site` only when True (keep `sites.toml` minimal). `kind` and shape validation unchanged. | ✅ | 2026-06-06 |
| TASK-002 | `tests/test_sites.py`: round-trip a `requires_browser=true` site (load→add→load); absent flag defaults False; non-bool value rejected; the flag does not interfere with feed/scrape shape validation. Run `mise run pre-commit`. Merge gate. | ✅ | 2026-06-06 |
| TASK-003 | (Deferred) The `add-site --requires-browser` CLI flag lands in **Phase 3** (TASK-011), not here: shipping it before the browser snapshot path exists would let a JS-site registration snapshot over httpx — a 200 challenge/interstitial body yields zero entries → an empty cold-start snapshot → a back-catalog flood on the first real run, violating main-plan REQ-002. Phase 1 ships only the data-model field. | n/a | 2026-06-06 |
