"""Site-health store — the durable per-site consecutive-failure counter.

This module owns all SQL against the ``site_health`` table created by the v4
migration in ``seen.py``, mirroring how ``forum_store`` owns the
...
(``cmd_new_entries``) share it verbatim.
...
parameterized SQL, no f-string value interpolation.
"""
...
def record_failure(conn: sqlite3.Connection, site_id: str) -> int:
    ...
    existing count. Commits on success. ``RETURNING`` reads back the
...
def record_success(conn: sqlite3.Connection, site_id: str) -> None:
    ...
    A reachable run clears the streak. No-op-safe when the site has
...
def is_persistent(count: int, threshold: int) -> bool:
    """True iff ``count`` consecutive failures has reached ``threshold``.

    Pure helper so the persistence decision is made once in Python and the skill
    never re-derives the threshold. The boundary is inclusive: a site
