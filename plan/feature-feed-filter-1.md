...
- **AMD-002** — **`sites.toml` is gitignored local state, not version-controlled.** Reverses ALT-005 / FILE-014, which had `sites.toml` committed alongside `selection.md`. The registry is a personal subscription list (personal data) and must never be committed; it is now gitignored like `feed-filter.db`. Only `selection.md` remains version-controlled config. (A migration commit briefly pushed it; it was removed — local history and the branch are purged, and the GitHub `refs/pull/<n>/head` copy requires GitHub Support to purge.)
- **AMD-003** — **The `Filtered Feeds` list holds only pages; operational notices go to a push notification.** Revises REQ-006's "report the change as a reminder in `Filtered Feeds`" and removes `reminders.report()` (TASK-027): self-heal, per-site gather errors, and the run summary are no longer filed as non-page list reminders. Instead the run routine ends by sending the summary via `PushNotification` (one line, ≤200 chars), with the full breakdown kept in the transcript as a fallback. Per-article wall/error entries still appear in the list — they are pages (a `--url` reminder whose note explains the issue), which is wanted. `heal-site` now files no reminder; its output is `{site_id, pattern, snapshotted}` and a non-zero exit is a single case (re-scrape failed before the config write).
- **AMD-004** — **The active selection prompt is gitignored local config; only a template is committed.** Supersedes ALT-005 and the closing sentence of AMD-002 ("Only `selection.md` remains version-controlled config"), which is now false. The selection criteria are personal config, so — like `sites.toml` and `feed-filter.db` — they are gitignored: the active prompt is `prompts/selection.md` (overridable via `FEED_FILTER_SELECTION`), and only `prompts/selection.example.md` (an English placeholder template) is version-controlled. A fresh checkout copies the example to `prompts/selection.md` and edits it. (An earlier attempt committed personal criteria in Japanese; reverted, and the repo was recreated to purge the GitHub `refs/pull/<n>/head` copy.)
