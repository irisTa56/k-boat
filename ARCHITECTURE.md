...
- **Site-health escalation (SH-REQ-001/003).** Each run is stateless, so it cannot tell a chronic outage from a one-run blip on its own. `site_health.py` persists a per-site consecutive-failure counter: `cmd_forum_new` increments it only when a site was **wholly unreachable** this run — every discovery feed raised `FetchError`, the typed `AdmitResult.all_feeds_failed` signal, never a heuristic parse of the error string — and resets it on any reachable run. A dead-topic retirement or a partial feed failure does **not** increment (SH-REQ-006). At `DEFAULT_PERSISTENT_FAILURE_RUNS` (3) the emitted `sites[]` entry flags `persistent`, and the forum-run skill escalates: it fires the push and recommends checking for a moved forum URL first, then `disable-site` if truly gone. The CLI never auto-disables — a persistent failure is as often a recoverable migration (the elixirforum subdomain move) as a dead site, so termination stays a human decision (SH-CON-003).
  This write is **operational telemetry, outside the never-lost dedupe authority** (SH-CON-002): it touches no `seen`/`forum_post_seen`/`forum_watch`/`completed_polls` state, so a crash after it costs at most one extra increment on a failure that would recur anyway — never a lost post. (Article-path parity — `cmd_new_entries` — is Phase 2 of `plan/feature-site-health-escalation-1.md`.)
