...
- **Default: `keep`.** Search sources with `keep == true` — the read-later shelf the user chose to hold (this covers `keep` alone and `keep` + `distill`).
  - These are the ones parked to read later.
- **Specify the states.** A `--states <list>` argument redirects the scope to the union of the named dispositions, drawn from `keep`, `distill`, `active`, `dismiss` (comma-separated, e.g. `--states keep,distill`).
  - `active` is the undispositioned inbox, computed as `!distill && !keep && !dismiss && !blocked` — there is no `active` property, so derive it from the flags.
  - Include `dismiss` only when the user names it explicitly: it is the abandoned pile, deliberately out of the default scope.
- **Always exclude `blocked` (DLQ) sources**, in every scope: they have no fetched content, empty `summary`/`topics`, and a `reading_link` that ingest already failed to open, so surfacing one as a readable result would mislead.
  - If a query clearly matches a blocked source by `title`/`url`, mention it separately as "in the DLQ — run `kboat-rescue` first", not as a readable hit.
...
1. Load the env with `eval "$(mise env)"` so `$OBSIDIAN_VAULT_PATH` is set from `.env` (see kboat-notes [Environment](../kboat-notes/SKILL.md#environment)).
   - Read the sources once, with only the fields the steps below use: `kboat-note list --type source --field title --field topics --field summary --field url --field source_type --field reading_link --field notebooklm_id --field gemini_url --field keep --field distill --field dismiss --field blocked`.
   - Name every `anomalies` entry beside the answer, whatever it is: this mode has no `kboat-doctor` ahead of it, and a shelf entry it did not see reads as one the reader never saved, so they save it again.
   - An exit 1 means `Sources/` could not be read, the entry under `Sources` saying how: say so and stop, rather than reporting an empty shelf.
...
4. Return the best matches, each with: `title`, `summary`, `source_type`, `reading_link`, and `url`.
   - Order by relevance.
   - If nothing clears the bar in scope, say so and offer to widen via `--states` (e.g. add `distill` or `active`).
...
### Step 1: gather the signals and the candidate pool

`eval "$(mise env)"`, then run `kboat-pick candidates` once — one JSON with both interest signals and the candidate pool:
...
### Step 3: infer the interests

Read the `questions` (by `rank`) and the `daily_notes` bodies (newest first) and infer what the reader wants to read or learn about now: an open question is the strongest, most deliberate signal, and its `rank` orders that signal (a smaller `rank` is a stronger interest); the notes add the topics, problems, and themes they are currently engaging with; ignore logbook noise (done tasks, schedules, unrelated journaling).

### Step 4: Stage 1 — local pre-filter (no NotebookLM)

Rank the candidates from their `summary`/`topics` alone into the three tiers of kboat-notes [Daily pick](../kboat-notes/references/daily-pick.md#daily-pick) — Tier 0 act-early above Tier 1 direct-interest above Tier 2 same-field learning; read it for what each tier admits and how it orders within — and keep a **shortlist** of the top handful (about three to five — wider than the cap so Stage 2 has room to choose two).

- Delegate to a cheap subagent when the inbox, the notes, or the question list are large, lexically pre-filtering first to keep it cheap; a Japanese note leans on the Japanese `summary`, since `topics` is English.
- If nothing clears any tier (no act-early candidate and nothing on-interest), the shortlist is empty: skip Stage 2 and go to step 6 with no picks.

### Step 5: Stage 2 — body-read final judgment (NotebookLM)

For each shortlisted candidate (a cheap subagent per candidate, in parallel), resolve its source id by matching its `url` (then `title`) in `notebooklm --quiet source list --notebook <notebooklm_id> --json 2>/dev/null` (the redirect kboat-notes [Environment](../kboat-notes/SKILL.md#environment) requires) — the original source, ignoring any saved dialogue notes (kboat-notes [One notebook per source](../kboat-notes/references/source-note.md#one-notebook-per-source-11)) — then `notebooklm --quiet source fulltext <source_id> --notebook <notebooklm_id> -o <tmpfile> --force` (use `-o`, not stdout, which truncates at 2000 chars; keep `--force`; and name `<tmpfile>` uniquely per candidate — its `source_id` is enough — since these subagents run in parallel and kboat-notes says why a shared name is unsafe).

- Search the extract for the body as kboat-notes' ingest fetch check says to ([Procedure: create or update a source note](../kboat-notes/references/procedures.md#procedure-create-or-update-a-source-note), step 3), and hand each subagent that rule with its task.
- Read the body, re-judge genuine relevance from it (demote or drop a candidate the summary oversold; confirm one that truly delivers), and record its length (the body's character count, not the extract's).
- Then choose at most two from the body-refined ranking, applying the two diversification tie-breakers of kboat-notes [Daily pick](../kboat-notes/references/daily-pick.md#daily-pick) — one older + one newer by `added_date`, and not two long reads (by the body length) — as tie-breakers only, never overriding relevance; read it for how they qualify and bound the pick to two.
- **Fallback:** if NotebookLM cannot be reached at all (auth unusable, the CLI failing), skip the body read and pick from the Stage 1 `summary`/`topics` ranking instead (Tier 0 included, so an act-early pick still fires), so the step still produces a pick.
...
- **Two outcomes are not fallbacks and must not be swallowed by one.**
...
- Lexical search only.
  - A source whose `summary`/`topics` are empty (the source guide failed at ingest) is findable only by `title`/`url`.
