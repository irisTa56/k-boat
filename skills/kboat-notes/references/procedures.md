...
1. Get the slug for the `url`: `kboat-note slug "<url>"`, and read `.slug` (same oracle as Conventions; `.canonical_url` is what it hashed).
   ...
   - Read what holds the slug, with the fields this step reads: `kboat-note list --type source --slug <slug> --field url --field blocked --field dismiss --field distilled_date --field notebooklm_id`.
     - Any `anomalies` entry is a note that cannot be read to de-dup against, not an absent one: one iCloud has evicted (the entry under its `Sources/.<slug>.md.icloud` placeholder), one that would not read or parse, or one holding one of those fields in a shape the reader does not model.
