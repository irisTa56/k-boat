...
1. Get the slug for the `url`: `kboat-note slug "<url>"`, and read `.slug` (same oracle as Conventions; `.canonical_url` is what it hashed).
   ...
   - Read what holds the slug with `kboat-note list --type source --slug <slug>`.
     - An `anomalies` entry and no note is a note that cannot be read to de-dup against, not an absent one: one iCloud has evicted (the entry under its `Sources/.<slug>.md.icloud` placeholder), or one that would not read or parse.
   ...
   - A matching note stops the item here in the first of these states it is in, with nothing fetched, built, or written; the caller deletes the queue file, the note on disk standing in for the capture:
     ...
     - **`dismiss: true`** → a dismissed source; the caller reports it as "already dismissed", telling the human to untick `dismiss` and capture the URL again if they want to read it.
       ...
       - While it stands, the dismiss branch discards the source's notebook once the cooldown has run — one a rebuild here would make, or the one a source still in its cooldown holds — so the capture is undone unless the human unticks it.
     - **`distilled_date` set and no `notebooklm_id`** → a distilled source whose notebook was discarded; the caller reports it as "already distilled", naming [Procedure: reactivate a source's notebook](#procedure-reactivate-a-sources-notebook) as the way to have a notebook for it again.
       - A rebuild here would give it a notebook nothing ever discards, `distilled_date` keeping it out of the ripe set; reactivation settles `distill` and the stamp before it builds.
...
3. Create the 1:1 notebook and record its coordinates:
   ...
   - Wait for the add to finish (adding is async): `notebooklm --quiet source wait <source_id> --notebook <id> --timeout 90 --json 2>/dev/null`. Branch on `.status`, **not** the exit code — the code merges `not_found` and `error` into `1`, and those two want opposite handling. Keep `--timeout` below the caller's own budget (the Bash tool allows 120s by default) so the CLI lives to report a timeout instead of being killed mid-wait, which would yield no status at all.
     ...
     - `error` → NotebookLM took the source and could not process it, and for a web source that verdict is durable: the library polls through an ERROR status only for a still-unclassified or media source, and treats it as terminal for every other type, a web page among them, so it will not report `error` for one that is merely still settling.
       - Record the source in the DLQ (see [Procedure: record a blocked source](#procedure-record-a-blocked-source-dlq)) as a `web_page`.
   ...
   - Confirm it **is** a web page: read `.source.type` from `notebooklm --quiet source get <source_id> --notebook <id> --json 2>/dev/null`.
     ...
     - Do **not** widen this to "any type that is not `web_page`": the schema has only these two values and rescue can supply only a PDF or captured text, so any other type (`youtube`, `epub`, `docx`, …) stays on the web path and is reported instead.
   ...
   - Confirm NotebookLM actually **fetched** the article.
     ...
     - **Search the extract for the article's body; do not judge it from its head.**
       ...
       - The text is a wall only where the whole extract holds no body: chrome with a sign-in, subscribe, JavaScript-required, or bot-check prompt, or the article's opening paragraphs cut off by one.
     - **A `wall` verdict must survive a second look before anything acts on it**: at ingest a false one discards the notebook that held the article and parks a readable source in the DLQ, where both exits are human-initiated.
       - Read the source guide's `.summary` and `.keywords` (`notebooklm --quiet source guide <source_id> --notebook <id> --json 2>/dev/null`).
       ...
       - Distillation's re-check reads the note's stored `summary` and `topics` as the guide wherever they are non-empty, and calls `source guide` only where both are empty.
         - They are this source's guide captured at ingest — its summary trimmed to a sentence or two, its keywords in English — and the second look uses a guide only to learn what to search the extract for.
     ...
     - This verification can run in a cheap subagent.
       - The subagent reads its task rather than this skill, so the task carries this bullet's rules on where to look and on the second look word for word, with the extract's path, the ids the guide call needs, and at distillation the stored `summary` and `topics` as well.
   ...
   - Update the note with `kboat-note write --type source` once more — a `{slug, fields}` record carrying `summary`, `topics`, `notebooklm_id`, and the derived `gemini_url`/`notebooklm_url` (derive both from `notebooklm_id`).
     ...
     - The returned NotebookLM source id is not stored; it is resolved on demand.
...
3. Resolve the `title`.
   - Prefer a clean human title over the PDF's internal one: for an arXiv PDF, read the abstract page (`/pdf/<id>` → `/abs/<id>`); otherwise use the PDF's metadata title, then its first-page heading, then the capture's link text.
   - Whenever the title falls back to the capture's link text, flag it so a human can fix it later (kboat-ingest reports this).
4. Create the note with `kboat-note write --type source`, exactly as on the web path (step 2): a `{slug, fields}` record with `type: source`, `title`, `source_type: pdf`, `url` = the queued URL, and `reading_link` = `[[<slug>.pdf]]`.
   - The tool defaults the booleans `false`, leaves `summary`/`topics`/`filed_date`/`distilled_date` empty, and stamps `added_date`; step 5 fills `summary`/`topics`.
   - This note write is the commit point.
5. Create the 1:1 notebook and record its coordinates:
   ...
   - Wait for the upload to process: `notebooklm --quiet source wait <source_id> --notebook <id> --timeout 90 --json 2>/dev/null`. Branch on `.status`, **not** the exit code — the code merges `not_found` and `error` into `1`, and those two want opposite handling. Keep `--timeout` below the caller's own budget (the Bash tool allows 120s by default) so the CLI lives to report a timeout instead of being killed mid-wait, which would yield no status at all.
     ...
     - `error` → NotebookLM took the file and could not process it, and that verdict is durable: the library polls through an ERROR status only for a still-unclassified or media source and treats it as terminal for every other type, an uploaded PDF among them.
       ...
       - Unlike an empty extraction, though, nothing here was ours to get wrong and nothing in the notebook is worth a look: the verdict is NotebookLM's own, and the source never reached `ready`, so the notebook holds one source that can never be processed or summarised.
       - Discard it (see [Procedure: discard a source's notebook](#procedure-discard-a-sources-notebook), passing the id `create` returned) and leave `notebooklm_id` empty rather than record a notebook that can answer nothing and would hold the source in the `needs_summary` retry for good.
       - Keep the note and file, leave `blocked` `false`, and report that NotebookLM rejected these bytes — what helps is a different or re-exported copy, not the *text-bearing* one an empty extraction calls for, since an errored PDF may carry good text and simply be corrupt, encrypted, or oversized.
         - The way back, once such a copy exists, is [Procedure: reactivate a source's notebook](#procedure-reactivate-a-sources-notebook).
       - kboat-ingest deletes the queue file, since a retry would re-upload the same bytes.
   ...
   - Once `ready`, verify the extraction.
     ...
     - Keep the note, file, and notebook (`blocked` stays `false`) and record `notebooklm_id` (via the update below).
       - The notebook stays because this verdict is **ours**, not NotebookLM's, and the notebook is the only place the text it was made on can be read: a PDF that extracts badly looks perfect in Obsidian — an image-only scan is exactly that — so opening the file settles nothing, and a human who wants to check the call has to open the notebook.
       - Keeping it leaks nothing, the note referencing it, which is the same ground that lets distillation keep one when its own re-check fails (step 3 of [create or update a source note](#procedure-create-or-update-a-source-note)).
       - It does cost what the `error` branch above refuses to pay — a `needs_summary` retry the guide may never satisfy — and that is the bargain: the same cost, against a benefit `error` has none of, since there the verdict is not ours and the notebook holds nothing to weigh it against.
       - A reactivation or a `dismiss` ends the retry by discarding the notebook.
     - Report the empty extraction so a human can supply a text-bearing copy if they want AI dialogue or distillation: they replace `PDFs/<slug>.pdf` with it and run [Procedure: reactivate a source's notebook](#procedure-reactivate-a-sources-notebook), which takes this source **with** its notebook, discards it first, and rebuilds from the file it finds.
       - Say in the same breath that `distill` is not the way to ask for that — set on this source it would go ripe and abort every run, distillation re-checking this very extraction and aborting without stamping or discarding.
     - This verification can run in a cheap subagent.
   - Capture the summary (see [Procedure: capture summary and topics](#procedure-capture-summary-and-topics)).
   - Update the note with `kboat-note write --type source` carrying `summary`, `topics`, `notebooklm_id`, and the derived `gemini_url`/`notebooklm_url` (the tool merges these over the note, preserving the rest).
     - The returned source id is not stored; it is resolved on demand.
...
A DLQ entry usually has none, but one can still carry the notebook an earlier ingest built — the `blocked_has_notebook` row — and such a source is not missing its content at all: the article is already inside.
...
A DLQ entry's dispositions are inert rather than absent and `filed_date` is not cleared while it is blocked (the lifecycle skips a blocked source in both phases), so a `dismiss` ticked before the entry was recorded, with a stamp already seven days old, puts this source in the **next run's discard set** the moment the flag clears — and the notebook goes with it.
...
3. **`notebooklm_id` empty, where the tombstone record is the one being written.** A DLQ entry usually has none, but one can still carry one (the `blocked_has_notebook` row).
...
The `reading_link` matters for the same reason — a `distilled_date` reaches the DLQ only on a note that already existed, and recording the entry over it overwrote the link with the `url`, which would then be what recall offers as the way to read it.
