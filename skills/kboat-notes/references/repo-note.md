...
| `gone` | Checkbox, set by the human; `false` by default. Ticked, it says GitHub no longer shows this account the repository — deleted or made private, which a 404 cannot tell apart — and the human is keeping the note anyway: `kboat-repos refresh` skips it, and it leaves the unrefreshed-repo [backlog stats](validation.md#backlog-stats). Unticked, the next refresh takes it up again. Only a human ticks it: a refresh `failed` entry with `reason: no_such_repo` is what puts the choice to them, and `refresh` never touches the field. It is cleared in one other way: when the repository is catalogued again, `kboat-repos write` writes `gone: false`, since that write follows a `gather` in which GitHub showed the repository, so the tick is known to be false by then. A rename is not this — refresh adopts renames. |
...
- `refreshed_date` advances each time `kboat-repos refresh` re-fetches the GitHub metadata and recomputes `status`.
  - It stops advancing on a note the refresh fails on, and on a note ticked `gone`, which the refresh skips.
- A repository GitHub no longer shows this account is reported by the refresh as `no_such_repo` on every run, and the note is kept until a human settles it: tick `gone` to keep the note, restore the access if the repository should still be visible, or delete the note.
  - A note ticked `gone` whose repository is captured again, and which GitHub shows again, is written with `gone: false` and refreshed from then on.
