...
Load the env with `eval "$(mise env)"` at the top of every shell block (see kboat-notes [Environment](../kboat-notes/SKILL.md#environment)): it sets `$OBSIDIAN_VAULT_PATH` and puts both venvs on `PATH`, so the `notebooklm` commands resolve bare.
